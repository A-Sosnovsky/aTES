...
public class TaskCompletedEventHandler : IMessageHandler<TaskCompleted>
{
    ...
    public TaskCompletedEventHandler(IServiceProvider serviceProvider)
    {
    ...
    public async Task Handle(IMessageContext context, TaskCompleted message)
    {
        ...
        var account = await repository.Query<Account>().FirstOrDefaultAsync(a => a.UserId == message.CompletedBy);
        ...
        var price = TaskPriceCalculator.CalculateCompetedPrice();
        var billingCycleId = await billingCycleService.GetCurrentBillingCycleId();

        task.AssignedToId = message.CompletedBy;
        ...
        await eventsService.TransactionCompleted(transaction.PublicId, account.PublicId, price, TransactionType.Enrollment);
    }
}